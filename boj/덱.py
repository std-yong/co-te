...
for _ in range(N):
    ...
    if cmd[0] == "push_front": # push_front일 때
        que.appendleft(int(cmd[1]))
        
    elif cmd[0] == "push_back": # push_back일 때
        que.append(int(cmd[1]))
        
    elif cmd[0] == "pop_front": # pop_front일 때
        if que: # 큐가 비어있는지 확인
            result.append(que.popleft())
        else:
            result.append(-1)
            
    elif cmd[0] == "pop_back": # pop_back일 때
        if que: # 큐가 비어있는지 확인
...
# 저번 큐_2.py 문제와 거의 같다
# 다른점은 appendleft와 popleft를 사용할 줄 아는가를 묻는점?